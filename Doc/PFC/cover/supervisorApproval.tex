%=====================================================================================%
% Author : Sánchez Barreiro, Pablo.                                                   %
% Version: 2.0, 11/05/2009                                                            %
% PhD dissertation: cover/supervisorApproval                                          %
%=====================================================================================%

\begin{tabular}{lcr}
	\includegraphics[width=0.10\linewidth]{images/ingInformatica.eps}
	& \ \ \ \ \ \ \ \ \ \ \ \
	\ \ \ \ \ \ \ \ \ \ \ \
	\ \ \ \ \ \ \ \ \ \ \ \
	\ \ \ \ \ \ \ \ \ \ \ \
	\ \ \ \ \ \ \ \ \ \ \ \
	\ \ \ \ \ \ \ \ \ \ \ \
	&
	\includegraphics[width=0.10\linewidth]{images/uc.eps} \\
\end{tabular}


\ \\ \ \\

El Dr. Ing. Don Pablo S{\'a}nchez Barreiro, Ayudante Doctor del {\'A}rea de Lenguajes y Sistemas Inform{\'a}ticos de la Facultad de Ciencias de la Universidad de Cantabria,

\ \\ \ \\ \ \\

Certifica que D. {\'A}ngel Tezanos Ib{\'a}{\~n}ez, alumno de Ingeniería Informática, ha realizado en el Departamento de Matemáticas, Estadística y Computaci{\'o}n de la Universidad de Cantabria, bajo mi direcci{\'o}n, el Proyecto Fin de Carrera titulado:

\ \\ \ \\

\begin{center}
\emph{Desarrollo de un Clasificador de Imágenes Digitales inspirado en los Clasificadores de Diapositivas de la Fotografía Analógica}
\end{center}

\ \\ \ \\

Revisado el presente trabajo, estimo que puede ser presentado al tribunal que ha de juzgarlo, y autorizamos la presentaci{\'o}n de este Proyecto Fin de Carrera en la Universidad de Cantabria.

\ \\ \ \\

\begin{flushright}
Santander, Junio de 2011
\end{flushright}

\begin{center}
\ \\ \ \\ \ \\ \ \\ \ \\ \ \\
Fdo.: Pablo S{\'a}nchez Barreiro \\
Ayudante Doctor del Área de Lenguajes y Sistemas Informáticos.
\end{center}


\thispagestyle{empty} \

