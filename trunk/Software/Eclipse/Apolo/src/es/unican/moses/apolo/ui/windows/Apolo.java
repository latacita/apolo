...
public class Apolo {
	...
	private void initialize() {
		...
		gui_estanterias = new GUIEstanteria(componentAdapter, gui_album);
		...
		frmApolo.getContentPane().add(gui_estanterias, gbc_estanterias);
		...
		mntmWeb.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				} catch (IOException e) {} catch (URISyntaxException e) {e.printStackTrace();}
			}
		});
	...
	public void exportar(){
		...
		if (lista == null || lista.size() == 0){
			JOptionPane.showMessageDialog(null, "Debe tener al menos una balda añadida", "Sin fotos que exportar", JOptionPane.INFORMATION_MESSAGE);
		}else{
	...
	public void cargar(){
		new DialogoCargarEstado();
	}
	
	public void guardar(){
		new DialogoGuardarEstado(frmApolo, gui_album, gui_estanterias, gui_mesa);
	}
