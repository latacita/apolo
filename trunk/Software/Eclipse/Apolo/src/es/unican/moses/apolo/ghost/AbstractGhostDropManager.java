...
public abstract class AbstractGhostDropManager implements GhostDropListener {
	...
	protected Point getTranslatedPoint(Point point) {
        ...
        if (Constantes.DEBUG) System.out.println("Punto capturado " +point);
        SwingUtilities.convertPointFromScreen(p, component);
        if (Constantes.DEBUG) System.out.println("Transformacion del punto " +p);
		return p;
	}
	...
	protected boolean isInTarget(Point point) {
		...
		if (Constantes.DEBUG) System.out.println("Rectangulo oyente " + bounds);
		if (Constantes.DEBUG) System.out.println("----------------------------");
