...
public class VentanaVisorImagenes extends JDialog {
	...
	private void inicializacion(){
		...
		cargaImagen(mesa.getDiapositiva(numfoto).getFichero());
		...
		
		{
			...
			buttonPane.setLayout(new BorderLayout(0, 0));
			{
				...
				btnSiguiente.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						if ((numfoto+1)<mesa.getNumeroDiapositivas()){
							cargaImagen(mesa.getDiapositiva(numfoto+1).getFichero());
							numfoto++;
						}else{
			...
			{
				JLabel lblNombreDeLa = new JLabel("Nombre: '" + mesa.getDiapositiva(numfoto).getNombre()+ "' ------ Tamaño: " + mesa.getDiapositiva(numfoto).getAnchura() + "x" + mesa.getDiapositiva(numfoto).getAltura() + "px");
